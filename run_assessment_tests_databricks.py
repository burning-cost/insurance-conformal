"""
Run insurance_conformal.assessment tests on Databricks serverless.
Tests: ConditionalCoverageAssessor, CVIAssessmentResult, CVISelectResult.
"""
import os
import base64
import time
...
workspace_base = "/Workspace/insurance-conformal-assessment-test"
...
for py_file in sorted(repo_root.rglob("*.py")):
    ...
    if any(p.startswith(".") for p in rel.parts[:-1]):
        continue
    if rel.name == "run_assessment_tests_databricks.py":
        continue
...
WS = "/Workspace/insurance-conformal-assessment-test"
TMP = "/tmp/insurance-conformal-assessment-test"
...
r2 = subprocess.run(
    [sys.executable, "-c",
     ...
     "from insurance_conformal.assessment import ConditionalCoverageAssessor, CVIAssessmentResult, CVISelectResult; "
     "print(\\'assessment imports OK\\')"],
...
# Run assessment tests only (fast — no LightGBM)
r3 = subprocess.run(
    [sys.executable, "-m", "pytest",
     f"{TMP}/tests/test_assessment.py",
     "-v", "--tb=short",
     "-p", "no:cacheprovider",
...
nb_path = f"{workspace_base}/run_assessment_tests"
w.workspace.import_(
    path=nb_path,
    content=nb_b64,
...
run = w.jobs.submit(
    run_name="insurance-conformal-assessment-tests",
    tasks=[
        Task(
            task_key="run_tests",
