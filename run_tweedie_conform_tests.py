"""
Run TweedieConformPredictor tests on Databricks serverless.
Uploads src + tests, installs dependencies, runs pytest.
"""
import os
import base64
import time
...
workspace_base = "/Workspace/insurance-conformal-tweedie-conform"
...
for py_file in sorted(repo_root.rglob("*.py")):
    ...
    if any(p.startswith(".") for p in rel.parts[:-1]):
        continue
    if rel.name in ("run_tweedie_conform_tests.py", "run_tests_databricks.py"):
        continue
...
# Copy from workspace (read-only) to /tmp (writable)
WS = "/Workspace/insurance-conformal-tweedie-conform"
TMP = "/tmp/insurance-conformal-tweedie-conform"
...
check_code = (
    ...
    "from insurance_conformal import TweedieConformPredictor; print('TweedieConformPredictor OK'); "
    "from insurance_conformal.claims import TweediePearsonScore; print('claims OK')"
)
r2 = subprocess.run(
...
if r2.returncode != 0:
    log.append("IMPORT_ERR:" + r2.stderr[-2000:])
    dbutils.notebook.exit("\n".join(log))
...
run = w.jobs.submit(
    run_name="insurance-conformal-tweedie-conform-tests",
    tasks=[
        Task(
            task_key="run_tests",
