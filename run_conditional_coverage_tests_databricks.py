"""
Run ConditionalCoverageERT tests on Databricks serverless.
Tests the new conditional_coverage.py module introduced in v0.7.0.
"""
import os
import base64
import time
...
workspace_base = "/Workspace/insurance-conformal-v070-test"
...
for py_file in sorted(repo_root.rglob("*.py")):
    ...
    if any(p.startswith(".") for p in rel.parts[:-1]):
        continue
    if rel.name == "run_conditional_coverage_tests_databricks.py":
        continue
...
WS = "/Workspace/insurance-conformal-v070-test"
TMP = "/tmp/insurance-conformal-v070-test"
...
r2 = subprocess.run(
    [sys.executable, "-c",
     ...
     "print(\\'ConditionalCoverageERT OK\\'); "
     "from insurance_conformal import ConditionalCoverageERT as C2; print(\\'top-level import OK\\')"],
...
# Run only conditional coverage tests
r3 = subprocess.run(
...
nb_path = f"{workspace_base}/run_conditional_coverage_tests"
w.workspace.import_(
    path=nb_path,
    content=nb_b64,
...
run = w.jobs.submit(
    run_name="insurance-conformal-v070-conditional-coverage-tests",
    tasks=[
        Task(
            task_key="run_tests",
