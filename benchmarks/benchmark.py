"""
Benchmark: insurance-conformal vs naive parametric intervals for insurance pricing.

The claim: for Tweedie/Poisson insurance data, the standard approach of using
a parametric interval (±k * global sigma) systematically undercovers high-risk
segments by 10-20 percentage points. Conformal prediction provides a distribution-free
coverage guarantee that holds regardless of residual distribution or model misspecification.

The key distinction: coverage guarantee is not the same as good-on-average coverage.
A naive parametric interval achieves ~90% aggregate coverage but only 70-75% coverage
for the most expensive 10% of policies — the tail that drives reinsurance and SCR.

Setup:
- 50,000 synthetic UK motor policies, known Gamma DGP (right-skewed severity)
- 60/20/20 temporal train/calibration/test split
- Sklearn Ridge baseline model (log-link, same for both methods)
...
Expected output:
- Naive parametric: ~90% aggregate, but 70-75% worst-decile coverage
- Conformal: >= 90% in every decile by construction (marginal guarantee)
- Conformal pearson_weighted intervals ~15-25% narrower than raw conformal
