"""
Run insurance-conformal tests (including new solvency_capital_range) on Databricks serverless.
Upload src + tests, install, run pytest -x.
"""
import os
import base64
import time
...
for py_file in sorted(repo_root.rglob("*.py")):
    ...
    if any(p.startswith(".") for p in rel.parts[:-1]):
        continue
    if rel.name in ("run_tests_databricks.py", "run_solvency_tests_databricks.py",
                    "run_benchmark_databricks.py", "run_benchmark_gbm_databricks.py",
                    "run_cqr_tests_databricks.py"):
        continue
...
# Sanity check — verify all subpackages importable including new v0.6.3 features
r2 = subprocess.run(
    [sys.executable, "-c",
     f"import sys; sys.path.insert(0, \\"{src_path}\\"); "
     "import insurance_conformal; print(insurance_conformal.__version__); "
     "from insurance_conformal import solvency_capital_range, SolvencyCapitalRange; print(\\'solvency OK\\'); "
...
# Run all tests with -x (fail-fast)
r3 = subprocess.run(
...
nb_path = f"{workspace_base}/run_solvency_tests"
w.workspace.import_(
    path=nb_path,
    content=nb_b64,
...
run = w.jobs.submit(
    run_name="insurance-conformal-v063-solvency-tests",
    tasks=[
        Task(
            task_key="run_tests",
